...
select t.brand,p.lifestage,p.premium_customer,round(sum(t.tot_sales),2) as total_sales
from transactions as t
join 
purchase_behaviour as p
ON t.LYLTY_CARD_NBR = p.LYLTY_CARD_NBR
group by t.brand,p.lifestage,p.premium_customer
order by total_sales desc; 
...
select 
brand,pack_size_g,round(sum(tot_sales),2) as total_sales
from transactions
group by  brand,pack_size_g
order by total_sales desc;
...
SELECT 
  (SUM(CASE WHEN t.brand = "Doritos" THEN t.prod_qty ELSE 0 END) 
  / SUM(t.prod_qty) )*100 AS kettle_pct
FROM transactions as t
join
purchase_behaviour as p
on t.LYLTY_CARD_NBR = p.LYLTY_CARD_NBR
...
SELECT 
  (SUM(CASE WHEN t.brand = "Doritos" THEN t.prod_qty ELSE 0 END) 
  / SUM(t.prod_qty) )*100 AS other_kettle_pct
FROM transactions as t
join
purchase_behaviour as p
on t.LYLTY_CARD_NBR = p.LYLTY_CARD_NBR
